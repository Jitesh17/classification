...
filename_pth = f'ckpt_densenet121_mark_correct_{IMG_SIZE}_s2.pth'
...
submission = pd.DataFrame({"id":fn_list, "label":pred_list})
submission.to_csv(f'preds_densenet121_dir_{IMG_SIZE}_test.csv', 
                #   index=False
                  )
samples, _ = iter(testloader).next()
samples = samples.to(device)
...
for num, sample in enumerate(samples[:24]):
    ...
    plt.imshow(np.transpose(sample, (1,2,0)))
# ax = plt.gca()
plt.savefig('inference_mark_direction_test.png')
# %%
