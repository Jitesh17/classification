...
# In[30]:
def  get_val(model, model_path, test_dir_path, ):
    ...
    test_list = [file for file in os.listdir(test_dir_path) if os.path.isfile(os.path.join(test_dir_path, file)) and "b1" in file]
...
# In[6]:
cat_files = [tf for tf in filename_list["train"] if "10" in tf]# if b_type in tf]
dir_path = os.path.join(dataset_path, "train")
...
images, _ = data.next()
# show images
show_img(torchvision.utils.make_grid(images))    # def __getitem__(self, idx):
# In[6]:
